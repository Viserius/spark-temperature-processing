...
if [[ ${action} = apply ]]; then
	...
	kubectl wait pod --for condition=ready -l component=zookeeper --timeout=300s --namespace=sc
	sleep 10s
fi
...
if [[ ${action} = apply ]]; then
	...
	kubectl wait pod --for condition=ready -l component=kafka-1 --timeout=300s --namespace=sc
	sleep 10s

	echo "Waiting for kafka-2 to become ready..."
	kubectl wait pod --for condition=ready -l component=kafka-2 --timeout=300s --namespace=sc
	sleep 10s

	echo "Waiting for kafka-3 to become ready..."
	kubectl wait pod --for condition=ready -l component=kafka-3 --timeout=300s --namespace=sc
	sleep 10s
